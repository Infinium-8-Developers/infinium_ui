...
export class AppComponent implements OnInit, OnDestroy {
  ...
  getMoneyEquivalent() {
    this.http.get('https://api.coingecko.com/api/v3/ping').subscribe(
      () => {
        this.http.get('https://api.coingecko.com/api/v3/simple/price?ids=infinium&vs_currencies=usd&include_24hr_change=true').subscribe(
          data => {
            this.variablesService.moneyEquivalent = data['infinium']['usd'];
            this.variablesService.moneyEquivalentPercent = data['infinium']['usd_24h_change'];
          },
          error => {
